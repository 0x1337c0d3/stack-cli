...
// Module-level: also used by EKReminder+Encodable.swift
func extractTags(from notes: String?) -> [String] {
    ...
    return regex.matches(in: notes, range: range).compactMap { match in
        Range(match.range(at: 1), in: notes).map { String(notes[$0]) }
    }
}

private func notesWithoutTags(_ notes: String?) -> String? {
    guard let notes = notes else { return nil }
    guard let regex = try? NSRegularExpression(pattern: "\\s*#\\w+") else { return notes }
    let range = NSRange(notes.startIndex..., in: notes)
    let stripped = regex.stringByReplacingMatches(in: notes, options: [], range: range, withTemplate: "")
        .trimmingCharacters(in: .whitespacesAndNewlines)
    return stripped.isEmpty ? nil : stripped
}

private func appendTags(_ tags: [String], to notes: String?) -> String? {
    guard !tags.isEmpty else { return notes }
    let tagString = tags.map { "#\($0)" }.joined(separator: " ")
    if let notes = notes, !notes.isEmpty {
        return "\(notes) \(tagString)"
    }
    return tagString
}
...
private func format(_ reminder: EKReminder, at index: Int?, listName: String? = nil) -> String {
    ...
    let tags = extractTags(from: reminder.notes)
    let tagsString = tags.isEmpty ? "" : " [\(tags.map { "#\($0)" }.joined(separator: " "))]"
    let notesString = notesWithoutTags(reminder.notes).map { " (\($0))" } ?? ""
    let indexString = index.map { "\($0): " } ?? ""
    return "\(listString)\(indexString)\(reminder.title ?? "<unknown>")\(notesString)\(tagsString)\(dateString)\(priorityString)"
}
...
public final class Reminders {
    ...
    func showAllReminders(
        ...
        displayOptions: DisplayOptions, tag: String?, json: Bool
    ) {
        let semaphore = DispatchSemaphore(value: 0)
        ...
        self.reminders(on: self.getCalendars(), displayOptions: displayOptions) { reminders in
            ...
            for (i, reminder) in reminders.enumerated() {
                ...
                if let tag = tag, !extractTags(from: reminder.notes).contains(tag) {
                    continue
                }
    ...
    func showListItems(
        ...
        displayOptions: DisplayOptions, tag: String?, json: Bool, sort: Sort, sortOrder: CustomSortOrder
    ) {
        let semaphore = DispatchSemaphore(value: 0)
        let calendar = Calendar.current
    ...
    func addReminder(
        ...
        reminder.notes = appendTags(tags, to: notes)
    ...
    func move(itemAtIndex index: String, fromListNamed sourceName: String, toListNamed destName: String) {
        let sourceCalendar = self.calendar(withName: sourceName)
        let destCalendar = self.calendar(withName: destName)
        let semaphore = DispatchSemaphore(value: 0)
        ...
        self.reminders(on: [sourceCalendar], displayOptions: .incomplete) { reminders in
            ...
            do {
                ...
                try Store.save(reminder, commit: true)
                print("Moved '\(reminder.title!)' to '\(destCalendar.title)'")
            } catch let error {
